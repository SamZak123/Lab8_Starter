describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    var value = innerText['_remoteObject'].value;
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const productItem = await page.$$('product-item');
    for(var i = 1; i < productItem.length; i++){
      var shadowRoot = await productItem[i].getProperty('shadowRoot'); 
      var button = await shadowRoot.$('button');
      await button.click();
    }
    var cart = await page.$('#cart-count');
    var cartText = await cart.getProperty('innerText');
    var value = cartText['_remoteObject'].value;
